...
import { getCollectedTimeFrame } from '../../../../redux/actions/collection'
import {Chart as ChartJS } from 'chart.js/auto'
import { Line } from 'react-chartjs-2'
import { useDispatch, useSelector } from 'react-redux'


const CollectedTimeFrame = () => {
    ...
    const { collectedTimeFrame } = useSelector((state) => state.collection);
    const [formData, setFormData] = useState({from: '', to: ''})

    var chartLabels = []
    var chartData = []

    if(collectedTimeFrame != null){
        collectedTimeFrame.forEach(populateLabel)

        function populateLabel(item){
        chartLabels.push(item._id)
        }

        collectedTimeFrame.forEach(populateData)
    ...
    const handleFilter = () => {
        dispatch(getCollectedTimeFrame(formData))
    }
            ...
            <Paper style={{ padding: '10px', borderRadius: '15px' }} elevation={6}>
                <div className={classes.card}>
                    <div className={classes.section}>
                        <center>
                            <Typography variant="h6" component="h6">All Waste Collected Filter by Time Frame</Typography>
...
export default CollectedTimeFrame